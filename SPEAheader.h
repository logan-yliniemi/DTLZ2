...
public:
    ...
    void take_agent(Evo_Agent_DTLZ2);
    Evo_Agent_DTLZ2 agent;
};

void SPEA_2_mem::take_agent(Evo_Agent_DTLZ2 a){
    agent=a;
}
...
private:
    ...
    void take_agent(Evo_Agent_DTLZ2, int);
    void final_archive();
};
...
void SPEA_2::take_agent(Evo_Agent_DTLZ2 A, int spot){
...
void SPEA_2::find_raw_fitness() {
    ...
    for(int i=0; i<superset.size(); i++){
        ...
        for(int z=0; z<superset.at(i).dominated_by.size(); z++){
            ...
            cout << "\t at a score of " << superset.at(i).coordinates.at(0) << "," << superset.at(i).coordinates.at(1) << "," << superset.at(i).coordinates.at(2);
            cout << " to " << superset.at(idomby).coordinates.at(0) << "," << superset.at(idomby).coordinates.at(1) << "," << superset.at(i).coordinates.at(2) << endl;
